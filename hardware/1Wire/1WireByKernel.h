...
#define MAX_DIGITAL_IO 8
	...
	{
	      public:
		explicit DeviceState(const _t1WireDevice &device)
			: m_Device(device)
		{
		}
		_t1WireDevice GetDevice() const
		{
			return m_Device;
		}
		union {
	...
	{
	      public:
		explicit Locker(const std::mutex &mutex)
			: std::lock_guard<std::mutex>(*(const_cast<std::mutex *>(&mutex)))
		{
		}
	...
	class IsPendingChanges
	{
		...
		std::list<DeviceState> &m_List;
	      ...
	      public:
		explicit IsPendingChanges(std::list<DeviceState> &list)
			: m_List(list)
		{
		}
		bool operator()() const
...
{
      public:
	explicit OneWireReadErrorException(const std::string &deviceFileName)
		: m_Message("1-Wire system : error reading value from ")
	{
		m_Message.append(deviceFileName);
	}
...
{
      public:
	explicit OneWireWriteErrorException(const std::string &deviceFileName)
		: m_Message("1-Wire system : error writing value from ")
	{
		m_Message.append(deviceFileName);
	}
